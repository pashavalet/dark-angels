...
export default function ServicesPage() {
  ...
  const allTags = useMemo(() => {
    ...
    data.data.forEach((service) => service.tags.forEach((tag) => tags.add(tag)));
    ...
    <div className="px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-text-muted">
          {t('explore_services', 'Explore our premium services')}
        </p>
      </div>
...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-text-muted">{t('no_services_found', 'No services found')}</p>
    </div>
  );
}
